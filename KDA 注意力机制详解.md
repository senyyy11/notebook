...
1. **先推导 Gated DeltaNet 到 KDA 的变化**：把标量遗忘门替换为通道级对角门，逐项检查状态形状和乘法方向，明确 KDA 的新增自由度。
2. **深入 chunkwise WY 表示**：从两个或四个时间步开始展开低秩状态转移，再理解块内并行怎样复现逐 token 递推结果。
3. **做关联记忆容量实验**：控制 $d_k$、$d_v$、头数、序列长度与 key 相似度，测量覆盖、碰撞、长期保留和精确复制能力。
4. **统一比较 KDA、RetNet、Mamba-2 与线性注意力**：写出各自的状态转移、输入依赖门控和读取公式，区分结构相似与机制等价。
5. **研究 KDA–MLA 混合层配置**：在长程检索、局部语言建模和精确复制任务上改变全局注意力间隔，观察固定状态与逐 token 缓存怎样互补。
