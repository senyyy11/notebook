...
建议沿“状态更新能力 → 并行训练 → 与其他长序列模型统一比较”的顺序继续：

1. **先深入 GLA、DeltaNet 与 Gated DeltaNet**：把它们的状态递推式并排展开，明确标量门、矩阵门和 Delta Rule 分别解决遗忘、覆盖与记忆碰撞中的哪一部分问题。
2. **再学习 chunkwise parallel 与 WY 表示**：理解为什么递推定义并不必然意味着训练只能逐 token 串行，以及块内并行怎样保持与递推计算等价。
3. **建立与状态空间模型的统一视角**：比较线性注意力、RetNet、Mamba/Mamba-2 的状态变量、输入依赖转移和读取方式，区分“关联记忆”与“状态空间滤波”两种解释。
4. **做一次可测量的实现对比**：在相同隐藏维度与序列长度下，对比基础线性注意力、Gated DeltaNet 和 FlashAttention 的显存、吞吐、首 token 延迟与长程检索准确率，观察渐近复杂度何时真正转化为工程收益。
